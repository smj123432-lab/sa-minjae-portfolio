...
- **무엇:** 프론트엔드 신입 개발자 사민재의 개인 포트폴리오 (싱글 페이지)
...
- **구조:** Hero → About → Skills → Projects(Diggo 메인 + 액티비오/사자들의 공부방 서브) → How I Work(AI 활용) → Contact
...
5. 컨텐츠(카피/수치)는 절대 임의로 창작하지 않는다 — 기획서 v3의 텍스트를 그대로 사용하거나, 빈 자리는 `TODO: 실제 수치로 교체` 주석으로 표시한다.
...
| `feature/*`  | 새 섹션·기능 추가                    | 예: `feature/hero-section`, `feature/ai-note-component`, `feature/theme-store` |
...
scope는 섹션/컴포넌트명(`hero`, `theme`, `metric-card`, `ai-note` 등)으로 통일.
...
- **파일/폴더:**
  ...
  - 폴더 구조: `app/` (라우트), `components/` (UI), `store/` (Zustand), `lib/` (유틸/타입), `content/` (카피·수치 데이터 분리 — 아래 참고)
- **콘텐츠 분리 원칙:** Diggo/액티비오/공부방의 텍스트·수치는 컴포넌트 안에 하드코딩하지 않고 `content/projects.ts` 같은 데이터 파일로 분리한다. 나중에 수치를 갱신할 때 컴포넌트를 건드릴 필요가 없어야 한다.
- **타입:** `interface`로 props 정의, 컴포넌트당 `ComponentNameProps` 네이밍.
- **Import 순서:** 외부 라이브러리 → 절대경로 내부 모듈(`@/components`, `@/store`) → 상대경로 → 타입.
- **'use client' 최소화:** 상태/이벤트가 필요한 컴포넌트(테마 토글, Reveal)만 클라이언트 컴포넌트로. 정적 콘텐츠 섹션은 서버 컴포넌트로 유지해 번들 크기를 줄인다.
...
- **접근성.** 키보드 포커스 아웃라인을 절대 `outline: none`으로 지우지 않는다(액센트 컬러로 커스텀 포커스 링은 가능). 이미지/아이콘에 `alt`, 토글 버튼에 `aria-pressed` 등 상태 속성 챙긴다.
