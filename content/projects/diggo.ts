...
export const DIGGO_TABS: DiggoTab[] = [
  ...
  },
  {
    ...
    label: '[02_N+1]',
    title: '채팅 목록 N+1 쿼리',
    imageSrc: '/images/diggo/diggo_chat_n1_query_fix.png',
    issue: [
      '배차 협의 채팅방이 늘수록 목록 로딩이 선형으로 느려지는 문제 발생',
      '채팅방 목록 조회 후 각 방마다 마지막 메시지·미읽음 수를 개별 쿼리로 추가 호출하는 구조',
      'N개 방에서 2N+3번 DB 호출 (채팅방 10개 = 23번, 20개 = 43번)',
    ],
    resolution: [
      '전체 채팅방 ID를 수집해 마지막 메시지·미읽음 수를 IN절 단일 쿼리로 일괄 조회',
      '결과를 room_id 기준 Map으로 변환해 채팅방 목록에 O(1)로 조립',
      '방 개수와 무관하게 총 쿼리 5개 고정 구조로 교체',
    ],
    result: [
      '채팅방 10개 기준 쿼리 수 23개 → 5개 (78% 감소)',
      '방이 늘어도 로딩 시간이 선형 증가하지 않고 일정하게 유지',
      '배차가 활발한 소장 계정에서 채팅 목록 진입 속도 체감 개선',
    ],
  },
  {
    id: 'ledger-bug',
